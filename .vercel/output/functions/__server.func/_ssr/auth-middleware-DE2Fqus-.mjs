import { o as getRequest, s as createMiddleware } from "./server-935pLRdS.mjs";
import { t as createClient } from "../_libs/supabase__supabase-js.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/auth-middleware-DE2Fqus-.js
...
var requireSupabaseAuth = createMiddleware({ type: "function" }).server(async ({ next }) => {
	...
	if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
		const message = `Missing Supabase environment variable(s): ${[...!SUPABASE_URL ? ["SUPABASE_URL"] : [], ...!SUPABASE_PUBLISHABLE_KEY ? ["SUPABASE_PUBLISHABLE_KEY"] : []].join(", ")}. Configure them in the server environment.`;
...
/** Optional auth for public flows such as guest checkout. */
var optionalSupabaseAuth = createMiddleware({ type: "function" }).server(async ({ next }) => {
	...
	return next({ context: data?.claims?.sub ? {
		supabase,
		userId: data.claims.sub,
		claims: data.claims
	} : {
...
//#endregion
export { requireSupabaseAuth as n, optionalSupabaseAuth as t };
