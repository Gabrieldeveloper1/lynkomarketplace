...
function reasonOf(decision: unknown): string | null {
  const d = decision as { reviews?: Array<{ comment?: string }>; warnings?: Array<{ short_description?: string }> } | null;
  if (!d) return null;
...
export const startKyc = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<KycState> => {
    ...
    if (!diditConfigured()) return { configured: false, status: "Not Started", sessionId: null, url: null };
    ...
    if (existing?.url && reusable.includes(existing.status)) {
      return { configured: true, status: existing.status, sessionId: existing.session_id, url: existing.url };
    }
    ...
    return { configured: true, status: "Not Started", sessionId: session.session_id, url: session.url };
  });
...
export const getKycState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<KycState> => {
    const { diditConfigured, getDiditDecision, applyDiditDecision } = await import("@/lib/didit.server");
    if (!diditConfigured()) return { configured: false, status: "Not Started", sessionId: null, url: null };
    ...
    try {
      ...
      if (freshStatus !== status) {
        await applyDiditDecision({ session_id: row.session_id, status: freshStatus, decision: fresh });
      }
      status = freshStatus;
    } catch (error) {
