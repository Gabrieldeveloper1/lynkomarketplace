...
export function HeroMarquee() {
  const { data: products = [] } = useQuery({
    queryKey: ["products", "hero-marquee"],
    queryFn: () => fetchProducts({ sort: "recentes", limit: 12, promotedFirst: true }),
  });
    ...
    return (
      <div className="grid h-64 place-items-center rounded-3xl border border-dashed border-border bg-card/60 p-8 text-center text-sm text-muted-foreground backdrop-blur-xl">
        <div>
          ...
          <p className="mt-3">Ainda não há anúncios publicados. Seja o primeiro a vender na Lynko!</p>
        </div>
      </div>
    );
  }
          ...
          {loop.map((p, i) => (
            ...
            >
              ...
              <div className="p-3">
                ...
                <p className="mt-1 text-sm font-extrabold text-primary">{formatPrice(p.price_cents)}</p>
              </div>
            </Link>
          ))}
        </div>
