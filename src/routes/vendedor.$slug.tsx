...
export const Route = createFileRoute("/vendedor/$slug")({
  head: ({ params }) => ({
    ...
    links: [
      { rel: "canonical", href: `https://www.lynkomarketplace.online/vendedor/${params.slug}` },
    ],
  }),
...
function SellerPage() {
  ...
  const {
    ...
    // O perfil desta rota é público. Não troque a consulta para `fetchSellerById`
    // quando o visitante é o próprio vendedor: isso dispara uma chamada server-side
    // de autenticação durante a hidratação e pode derrubar a rota para o proprietário.
  ...
  useEffect(() => {
    ...
    // O AuthProvider já mantém um canal `presence:${user.id}` para o usuário
    // logado. Usar outro nome aqui evita colisão quando o dono abre o próprio
    // perfil, cenário que anteriormente derrubava a rota inteira.
    const channel = supabase
      .channel(`seller-presence:${seller.id}`, { config: { presence: { key: seller.id } } })
      .on("presence", { event: "sync" }, () => {
        setPresenceOnline(Object.keys(channel.presenceState()).length > 0);
      })
      .on("presence", { event: "join" }, () => setPresenceOnline(true))
  ...
  if (seller.banned) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-24 text-center">
        <h1 className="text-2xl font-bold">Este perfil está banido por tempo indeterminado</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Este vendedor não está disponível na área pública do marketplace.
        </p>
      </div>
    );
  }
  ...
  const sellerCreatedAt = seller.created_at || new Date().toISOString();
  ...
  const lastSeenMs = lastSeen?.getTime() ?? Number.NaN;
  const online =
    user?.id === seller.id ||
    presenceOnline ||
    (Number.isFinite(lastSeenMs) && Date.now() - lastSeenMs < 5 * 60_000);
      ...
      <div className="mx-auto max-w-7xl px-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              >
                ...
                {online
                  ? "Online agora"
                  : lastSeen
                    ? `Offline ${timeAgo(seller.last_seen_at!)}`
                    : "Offline"}
              </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-xl border border-border bg-card p-3 text-center">
            <p className="text-lg font-extrabold text-emerald-500">{positiveReviews}</p>
            <p className="text-[11px] text-muted-foreground">Positivas</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-3 text-center">
            <p className="text-lg font-extrabold text-muted-foreground">0</p>
            <p className="text-[11px] text-muted-foreground">Neutras</p>
          </div>
          <div className="rounded-xl border border-border bg-card p-3 text-center">
            <p className="text-lg font-extrabold text-destructive">{negativeReviews}</p>
            <p className="text-[11px] text-muted-foreground">Negativas</p>
          </div>
        </div>
