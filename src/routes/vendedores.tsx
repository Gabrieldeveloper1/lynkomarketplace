...
import { VERIFICATION_LEVELS, levelLabel } from "@/lib/verification";
...
const FILTERS = [
  ...
  ...VERIFICATION_LEVELS.map((l) => ({ id: l.id, label: `Nível ${l.short}` })),
];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Perfis públicos da comunidade LynkoMarketplace, com nível de verificação de identidade.
      </p>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap gap-2" role="group" aria-label="Filtrar por nível de verificação">
          {FILTERS.map((f) => (
            <Button
              key={f.id}
              size="pill"
      ...
      {isLoading ? (
            ...
            <li key={s.id}>
              ...
              >
                ...
                <p className="mt-3 flex items-center justify-center gap-1 font-semibold">
                  ...
                  {s.verified && <BadgeCheck className="h-4 w-4 text-primary" aria-label="Vendedor verificado" />}
                </p>
                ...
                <p className="mt-3 inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-[11px] text-muted-foreground">
                  ...
                  {s.verified ? levelLabel(s.verification_level) : "Não verificado"}
                </p>
              </Link>
            </li>
          ))}
        ...
        <p className="mt-10 text-sm text-muted-foreground">Nenhum vendedor encontrado com esse filtro.</p>
      )}
    </div>
  );
}
