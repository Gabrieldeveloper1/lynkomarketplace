    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:py-10">
      ...
      >
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          {paid
            ? "O pagamento foi confirmado e o valor ficou protegido até você confirmar que recebeu tudo certo."
      ...
      <section className="mt-5 rounded-2xl border border-border bg-card p-4 sm:p-5">
        <div className="flex gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-display text-base font-bold">{product?.title ?? "Produto"}</p>
            {order.variant_name && <p className="truncate text-xs text-muted-foreground">{order.variant_name}</p>}
      ...
      <section className="mt-4 rounded-2xl border border-border bg-card p-4 sm:p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl bg-accent/50 p-3 text-[11px] text-muted-foreground">
          ...
          <span className="min-w-0">
            Pago por Pix via Efí Bank{order.charge_id ? ` · cobrança ${order.charge_id}` : ""}. O valor fica em
      ...
      <section className="mt-4 grid gap-2 sm:grid-cols-2">
        <Button asChild className="h-11 bg-gradient-primary text-primary-foreground">
          <Link to="/mensagens" search={{ c: undefined }}>
            <MessageCircleQuestion className="mr-2 h-4 w-4" /> Falar com suporte
          </Link>
        </Button>
      ...
      <p className="mt-4 text-center text-xs text-muted-foreground">
        ...
        e abra a disputa antes de confirmar o recebimento.
      </p>
    </div>
  );
}
