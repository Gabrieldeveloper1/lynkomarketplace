...
function Notificacoes() {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data as Notification[]).filter((notification) => notification.kind !== "order");
    },
  });
  ...
  const openNotification = async (notification: Notification) => {
    ...
    if (link.startsWith("/mensagens")) {
      ...
      navigate({
        to: "/mensagens",
        search: { c: url.searchParams.get("c") ?? undefined },
      });
      return;
    }
    if (link.startsWith("/pedido/")) {
      navigate({ to: "/pedido/$id", params: { id: link.slice("/pedido/".length) } });
      return;
    }
    if (link === "/dashboard") {
...
function PreferencesPanel() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("messages, withdrawals, verification, reports, marketing")
      .eq("user_id", user.id)
      .maybeSingle()
      .then(({ data }) => {
  ...
  return (
    <aside className="h-fit rounded-2xl border border-border bg-card p-5">
      <h2 className="flex items-center gap-2 text-sm font-semibold">
        <SlidersHorizontal className="h-4 w-4 text-primary" aria-hidden /> Preferências de
        notificação
      </h2>
