...
function PedidoPage() {
  ...
  const { data: order, isLoading } = useQuery({
    ...
    refetchInterval: (q) =>
      (q.state.data as { status?: string } | undefined)?.status === "pending" ? 10000 : false,
  });
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-20 text-center">
        ...
        <Button asChild className="mt-4">
          <Link to="/dashboard">Voltar à dashboard</Link>
        </Button>
      </div>
    );
  }
  ...
  const openDeliveryConversation = async () => {
    ...
    try {
      ...
      await navigate({ to: "/mensagens", search: { c: conversationId } });
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Não foi possível abrir a conversa da entrega.",
      );
    } finally {
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs font-medium ${done ? "" : "text-muted-foreground"}`}>
                  {s.label}
                </span>
              </li>
            );
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">
                Produto{order.variant_name ? ` · ${order.variant_name}` : ""}
              </span>
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          ...
          {order.status === "pending" ? (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Assim que o pagamento for confirmado pelo Efí Bank, o estado atualiza-se aqui
                automaticamente.
              </p>
              {pixCode && !pixCode.startsWith("http") ? (
                <div className="rounded-2xl border border-border bg-accent/40 p-4">
                  ...
                  <div className="mt-3 flex justify-center">
                    <div className="rounded-xl bg-white p-3">
                      {pixQrCode ? (
                        <img src={pixQrCode} alt="QR Code Pix do pedido" className="h-44 w-44" />
                      ) : (
                  ...
                  <div className="mt-3">
                    <p className="mb-1 text-xs font-semibold text-foreground">Pix copia e cola</p>
                    <p className="break-all rounded-lg bg-background p-2 text-[10px] text-muted-foreground">
                      {pixCode}
                    </p>
                  </div>
                  <Button
                ...
                <p className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-200">
                  O QR Code Pix ainda não foi gerado. Atualize a página ou tente gerar o pagamento
                  novamente.
                </p>
              )}
            </div>
            ...
            <p className="text-sm text-muted-foreground">
              Pagamento confirmado. O vendedor vai entregar em breve fale com ele nas mensagens se
              precisar.
            </p>
          )}
          ...
          <Button
            ...
            disabled={
              openingConversation || order.status === "pending" || order.status === "cancelled"
            }
          ...
          >
            ...
            {order.status === "pending"
              ? "Conversa disponível após o pagamento"
