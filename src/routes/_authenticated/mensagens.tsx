...
function Mensagens() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "messages" }, () => {
        void refetch();
        void qc.invalidateQueries({ queryKey: ["conversation-previews", user.id] });
  ...
  const meta = useMemo(() => {
    ...
    for (const m of recent) {
      ...
      // A conversa aberta é considerada lida imediatamente. O ChatPanel
      // persiste read_at no banco, mas este estado local evita que o selo
      // continue mostrando "1", "2"... até a próxima consulta.
      if (!m.read_at && m.sender_id !== user?.id && m.conversation_id !== activeId)
        entry.unread += 1;
      map.set(m.conversation_id, entry);
    }
    return map;
  }, [recent, user?.id, activeId]);
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            Converse com compradores e vendedores. Pode responder mensagens específicas, denunciar
            abusos ou chamar a moderação a qualquer momento.
          </p>
        </div>
        {totalUnread > 0 && (
          <Badge className="h-7 gap-1 px-3">
