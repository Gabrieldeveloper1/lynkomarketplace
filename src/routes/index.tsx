    ...
    items: [
      {
        ...
        a: "1) Escolha o anúncio e a variação (ex.: 1, 2 ou 3 dias). 2) No checkout, selecione o nível de proteção. 3) Pague via Pix com QR Code ou copia e cola. 4) O pagamento é confirmado automaticamente. 5) O produto é entregue na página do pedido e você acompanha cada etapa em tempo real.",
      },
      {
        q: "Como funciona a entrega automática?",
        a: "Quando o anúncio tem entrega automática, o vendedor carrega o estoque (chaves, contas, códigos) na dashboard. Assim que o pagamento é confirmado, o conteúdo é entregue na página do pedido em segundos, sem intervenção humana.",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "O saldo aparece na aba Carteira da dashboard. Você pede o saque (mínimo R$ 3,50) com a sua chave Pix, a equipe analisa e o status aparece em tempo real no histórico.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        a: "São três: Básico (sem documentos — apenas contato e localidade), Intermediário (documento de identidade, sem selfie) e Completo (documento + selfie). Você escolhe o nível conforme o que se sente confortável em compartilhar.",
      },
      {
        q: "Quais dados aparecem no meu perfil público?",
        a: "Apenas o emblema de verificado, o nível alcançado e informações não sensíveis que você informou (país, cidade, nome comercial e rede social). Foto do documento, selfie, número do documento e telefone nunca são públicos — só a equipe de análise acessa.",
      },
      {
        q: "O que é o nível de proteção e por que aparece no checkout?",
        a: "É a nossa taxa de serviço. Escolha no checkout entre Básica (+R$ 0,10), Média (+R$ 0,50) ou Máxima (+R$ 2,00). O valor é fixo em centavos, independentemente do preço do produto — muda apenas a rapidez da mediação e a cobertura de reembolso.",
      },
    ],
  },
];
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "LynkoMarketplace — Produtos digitais com entrega automática" },
      {
...
function Grid({ loading, items }: { loading: boolean; items: Awaited<ReturnType<typeof fetchProducts>> }) {
  if (loading) {
    return (
  ...
  if (!items.length) {
    return (
      <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
        ...
        <div className="mt-4">
          <Link to="/dashboard">
            <Button className="bg-gradient-primary text-primary-foreground">Publicar anúncio</Button>
          </Link>
        </div>
      </div>
    );
...
function Home() {
  ...
  const profileCtas: {
  ...
  }[] = [
    {
      ...
      icon: <Search className="h-5 w-5" />,
      points: ["Entrega automática na página do pedido", "Pagamento em custódia até a entrega", "Chat direto com o vendedor"],
      link: (
    ...
    },
    {
      ...
      points: ["Variações com preço e estoque próprios", "Saldo na carteira a cada venda", "Saque mínimo de R$ 3,50"],
      link: (
    ...
    },
    {
      ...
      icon: <ShieldCheck className="h-5 w-5" />,
      points: [
        "Básico sem enviar documentos",
        "Intermediário com documento de identidade",
        "Completo com documento e selfie",
      ],
      link: (
        <Button asChild variant="outline" className="w-full">
          <Link to={user ? "/verificacao" : "/auth"}>
            Ver níveis de verificação <ArrowRight className="ml-1 h-4 w-4" />
          </Link>
        </Button>
      ),
    },
    ...
    <div>
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div
          ...
          aria-hidden
        />
        <div className="relative mx-auto max-w-5xl px-4 py-14 text-center sm:py-24">
          <Badge className="mb-6 gap-1 bg-primary/15 text-primary hover:bg-primary/20">
            <Zap className="h-3 w-3" /> Entrega automática 24/7
          </Badge>
          <h1 className="mx-auto max-w-4xl font-display text-[2rem] font-extrabold leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
          ...
          <div className="mt-7 grid gap-3 sm:flex sm:flex-wrap sm:justify-center">
            <Link to="/produtos" search={{ q: "", cat: "todas", sort: "recentes" }} className="w-full sm:w-auto">
              <Button size="lg" className="w-full gap-2 bg-gradient-primary text-primary-foreground shadow-glow sm:w-auto">
          ...
          <div className="mx-auto mt-9 grid max-w-2xl grid-cols-3 gap-3 sm:mt-12 sm:gap-4">
            {[
      ...
      <Section
        title="Anúncios em destaque dos vendedores"
        subtitle="Impulsionados e mais vendidos por quem anuncia na plataforma."
      ...
      <Section
        ...
        subtitle="Valor fixo em centavos, não uma percentagem do seu produto. Você escolhe o nível de proteção — todos valem a pena."
