...
export const Route = createFileRoute("/p/$slug")({
  head: ({ params }) => {
    ...
    return {
      meta: [
        { title },
        { name: "description", content: "Termos, políticas e informação legal da LynkoMarketplace." },
        { property: "og:title", content: title },
        { property: "og:description", content: "Termos, políticas e informação legal da LynkoMarketplace." },
...
function PaginaLegal() {
  ...
  if (!page) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <Button asChild className="mt-4"><Link to="/">Voltar ao início</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="pb-12">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-10 sm:py-14">
          ...
          <div className="mt-5 flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <h1 className="mt-1 text-2xl font-extrabold leading-tight sm:text-4xl">{page.title}</h1>
              {page.summary && (
                <p className="mt-2 max-w-2xl text-sm text-muted-foreground sm:text-base">{page.summary}</p>
              )}
      ...
      <div className="mx-auto mt-8 grid max-w-6xl gap-8 px-4 lg:grid-cols-[240px_1fr]">
        ...
        <article className="rounded-3xl border border-border bg-card p-5 shadow-card sm:p-8">
          <Content text={page.content} />
          <div className="mt-8 rounded-2xl bg-accent/60 p-4 text-xs text-muted-foreground">
            Ficou com dúvida sobre este documento? Fale com o suporte pelo chat da sua conta — a equipe
